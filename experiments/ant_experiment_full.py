...
def run_comprehensive_ant_experiment(num_trials=5, num_episodes=1000, save_dir="results/comprehensive_ant"):
    """Run comprehensive Ant transfer experiment with multiple trials."""
    
    # Results collection
    all_trial_results = {
        'baseline': [],
        'parameter_transfer_reduced_dof': [],
        'feature_transfer_planar': [],
        'policy_distillation_simple_physics': [],
...
def create_experiment_config(num_episodes=1000, seed=42):
    ...
    return {
        ...
        'eval_episodes': 10,   # Use 10 episodes for each evaluation for stability
        ...
        # Agent configuration with optimized hyperparameters
        'agent_config': {
            'type': 'actor_critic',
        ...
        'transfer_configs': [
            # Parameter transfer from reduced DOF Ant
            {
                ...
                'source_episodes': 500,  # Increased source training
                'mechanism_config': {
            ...
            # Feature transfer from planar Ant
            {
                ...
                'mechanism_config': {
                    ...
                    'layers_to_transfer': ["0", "2"],  # Transfer early layers
            ...
            # Policy distillation from simplified physics Ant
            {
            ...
            # Progressive transfer with intermediate environments
            {
                ...
                'mechanism_config': {
                    ...
                    'progressive': True,
                    'intermediate_envs': [
                        # Step 1: Reduce control frequency
                        {
                            'type': 'simplified_physics_ant',
                            'friction': 0.9,
                            'gravity': -9.5,
...
def visualize_aggregated_results(aggregated_metrics, save_dir):
    ...
    for metric in metrics:
        ...
        # Create boxplot
        plt.boxplot(boxplot_data, labels=methods, patch_artist=True,
...
def main():
    ...
    num_episodes = 1000  # Run 1000 episodes per experiment
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_dir = f"results/comprehensive_ant_{timestamp}"
    ...
    # Run experiment
    all_results = run_comprehensive_ant_experiment(
        num_trials=num_trials,
        num_episodes=num_episodes,
        save_dir=save_dir
    )
