...
class ExpectedSarsaAgent(BaseAgent):
    """Expected SARSA agent implementation."""
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        if done:
            ...
            # Probability of selecting each action under epsilon-greedy
            greedy_prob = 1.0 - self.exploration_rate + self.exploration_rate / self.action_space.n
            non_greedy_prob = self.exploration_rate / self.action_space.n
            
            # Calculate expected value
            expected_q = 0
            for a in range(self.action_space.n):
                if a == best_action:
                    expected_q += greedy_prob * next_q_values[a]
                else:
                    expected_q += non_greedy_prob * next_q_values[a]
            
            future_q = expected_q
        
        # Calculate new Q-value using Expected SARSA update rule
